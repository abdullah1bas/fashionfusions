      ...
      <Box sx={{ pt: 2, mt: 2.5, pr: {xs: 0, md: 2.5}, display: "flex", alignItems: "center", gap: 2 }} >
            ...
            {mySlider.map((item) => {
              return (
                <SwiperSlide key={item.link} className={`${styles.parentSlider}`}>
                  {/* priority={true} de lazem lw al img in heigh view ,but low view use loading */}
                  <Image priority={true} width={850} height={500} src={item.link} alt="swiper image" />
        ...
        <Box sx={{ display: { xs: "none", md: "flex" , flexDirection: 'column', gap: 8, minWidth: "26.6%" } }}>
          {cardData.map((item) =>(
            ...
            <Image priority={true} width={300} height={250} src={item.img} alt="image product" />
